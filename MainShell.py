...
def pagesetter(L):
    sure_isim = input(f"Please enter the name of sure: ").lower()
    while sure_isim not in L:
        sure_isim = input(f"Please enter the name of sure: ").lower()
    sure = L.index(sure_isim)
    ayet = input('Please enter the ayet number: ')
    driver.get(f"https://www.kuranmeali.com/AyetKarsilastirma.php?sure={sure+1}&ayet={ayet}")

def cevirisecici(s):
    L = ['golpinarli','aakgul','aparliyan','ahmettekin','ahmetvarol','abulac','alifikriyavuz','bahaeddinsaglam','bayraktar','besimatalay','cemalkulunkoglu','cemilsaid','diyanetislerieski','diyanetisleriyeni','kuranyolu','diyanetvakfi','edip','elmalilisade','elmaliliorj','demiyent','erhanaktas','hbasricantay','hayrat','ilyasyorulmaz','baltacioglu','ismailhakkiizmirli','ismailyakit','kadirecelik','mahmutkisa','mahmutozdemir','mehmetcakir','mehmetcoban','mehmetokuyan','mehmetturk','muhammedesed','mustafacavdar','islamoglu','orhankuntman','osmanfirat','omernasuhi','syildirim','sates','suleymantevfik','abayindir','spiris','simsek','ynuri','eskianadoluturkecsi','satiralti','bunyadov','pickthall','yusufali']
    if s in L:
        ind = L.index(s)
        return driver.find_element(By.ID,L[ind])
